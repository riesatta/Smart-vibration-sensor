...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	 ...
	 neai_classification(acc_buffer, output_class_buffer, &class_output);

	 sprintf((char *)buf, "\nClass Detected: %s. (Certainty: %d%%)\n", id2class[class_output],(uint16_t)(output_class_buffer[class_output-1]*100));
//	 sprintf((char *)buf, "\nSpeed Detected: %s. \n", id2class[newClass]);
